...
if($toiminto == "add") {
        $query = "INSERT INTO henkilo (nimi, username, admin) VALUES ('" . $nimi . "','" . $username . "'," . $admin . ")";
...
<p>Tässä näkymässä pääset hallitsemaan olemassaolevia henkilöitä ja lisäämään uusia.</p>
<table>
<tr>
<th>ID</th><th>Henkilö</th><th>Käyttäjätunnus</th><th>Admin</th>
</tr>
<?php
...
while($row = $result->fetch_row()) {
	...
	echo'></td><td><button type="submit" name="toiminto" value="update">Päivitä</button></td><td><button type="submit" name="toiminto" value="delete" onClick="return confirm(\'haluatko varmasti poistaa henkilön?\')">Poista</button></td></tr></form>';
}
?>
<form action="henkilo.php" method="post"><td>&nbsp;</td><td><input type="text" name="nimi"></td><td><input type="text" name="username"></td><td><input type="checkbox" name="admin"></td><td colspan="2"><button type="submit" name="toiminto" value="add">Lisää</button></td></tr></form>
</table>
<br />
